...
class Board extends DB {
  ...
  public function troFindAll() {
    $sql = 'SELECT  t.id,t.title,t.body,t.user_id,t.created_at,u.user_name,u.img ,count(c.id) FROM troubles t JOIN users u ON t.user_id = u.id left join comments c on c.trouble_id = t.id GROUP BY t.id desc';
    $stmt = $this->connect->prepare($sql);
    $stmt->execute();
  ...
  public function delete_com($id = null){
    ...
    //コメント数表示(trouble_detail)
    public function com_count($trouble_id){
      $sql = 'SELECT count(c.id) FROM troubles t JOIN comments  c ON t.id = c.trouble_id join users u on t.user_id = u.id where t.id = :trouble_id';
      $stmt = $this->connect->prepare($sql);
    ...
    public function troSarch($keyword) {
      $sql = "SELECT * FROM troubles where body or title LIKE '%$keyword%'";
      $stmt = $this->connect->prepare($sql);
      $stmt->execute();
      $result = $stmt->fetchAll();
      return $result;
