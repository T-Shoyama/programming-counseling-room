...
public function add($arr) {
  ...
  public function edit($arr) {
    $sql = 'UPDATE users SET user_name = :user_name, mail = :mail, img = :img WHERE id = :id';
    $stmt = $this->connect->prepare($sql);
    $params = array(
      ':id' => $arr['id'],
      ':user_name' => $arr['user_name'],
  ...
  public function myTroFind($id) {
    $sql = 'SELECT t.id,t.title,t.created_at,count(c.id) FROM troubles t left join comments c on c.trouble_id = t.id WHERE t.user_id = :user_id GROUP BY t.id DESC' ;
    $stmt = $this->connect->prepare($sql);
    $params = array(
      ':user_id' => $id,
    );
  ...
  //ユーザーコメント数表示(mypage)
  public function com_count_mine($user_id){
    $sql = 'SELECT count(c.id) FROM troubles t JOIN comments  c ON t.id = c.trouble_id join users u on t.user_id = u.id where u.id = :user_id';
    $stmt = $this->connect->prepare($sql);
    $params = array(':user_id' => $user_id);
    $stmt->execute($params);
  ...
  public function keep_trouble($user_id) {
    $sql = 'SELECT t.title, t.created_at, t.id, u.user_name,count(c.id) FROM troubles t JOIN users u ON t.user_id = u.id join keeps k  on k.trouble_id = t.id left join comments c on c.trouble_id = t.id WHERE k.user_id = :user_id GROUP BY t.id desc' ;
    $stmt = $this->connect->prepare($sql);
    $params = array(':user_id' => $user_id);
    $stmt->execute($params);
    $result = $stmt->fetchAll();
